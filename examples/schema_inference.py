...
def main():
    ...
    for col in schema.columns:
        print(
            f"  {col.name}: {col.redshift_type.value}"
            + (f"({col.length})" if col.length else "")
            + (" NOT NULL" if not col.nullable else "")
        )

    # With auto-suggested keys
    schema = inferer.infer_schema(df, "products", auto_suggest_keys=True)
    ...
    # Schema inference is local: it reads the DataFrame, not the cluster.
    # The connection parameters below are never used.
    fs = FrameShift(
        host="unused-for-inference",
        database="unused",
        user="unused",
        password="unused",
    )
